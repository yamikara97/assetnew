    ...
    public class VehicleController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var assetList = await (from asset in _context.Assets
                                   ...
                                   where j3.Id.Equals(selectID) && j1.AssetTypeCode == "LTS-PT"
        ...
        public async Task<IActionResult> Create(Guid? id)
        {
            ...
            var type = _context.AssetTypes.Where(a => a.AssetTypeCode == "LTS-PT").FirstOrDefault();
        ...
        public async Task<IActionResult> Create(Guid? id, [Bind("Name,Amount,MFG,Place,Address,Reason,Guarantee,DateUse,TechnicalData,Status,Note,Id,DateUpdate,UpdateBy,Wattage,WeightHandle,VehiclePlate,SeatNumber,BudgetSource,CareerSource,AidSource,AnotherSource")] Assets assets, IFormCollection collect)
        {
            ...
            assets.Type = _context.AssetTypes.Where(a => a.AssetTypeCode == "LTS-PT").FirstOrDefault();
                        ...
                        using (var package = new ExcelPackage(stream))
                        {
                            ...
                            if (checkExcel(worksheet))
                            {
                                for (int i = 2; i <= rowCount; i++)
                                {
                                    ...
                                    asset.Type = _context.AssetTypes.Where(a => a.AssetTypeName.Contains("vận tải khác")).FirstOrDefault();
                                    ...
                                    {
                                        asset.SeatNumber = int.Parse(worksheet.Cells[i, 14].Value.ToString());
                                    }
                                    if (worksheet.Cells[i, 16].Value != null)
                                    {
                                        asset.VehiclePlate = worksheet.Cells[i, 14].Value.ToString();
                                    }
                                    if (worksheet.Cells[i, 17].Value != null)
                                    {
                                        asset.WeightHandle = float.Parse(worksheet.Cells[i, 14].Value.ToString());
                                    }
                                    if (worksheet.Cells[i, 18].Value != null)
                                    {
                                        asset.Wattage = worksheet.Cells[i, 14].Value.ToString();
                                    }
        ...
        public bool checkExcel(ExcelWorksheet worksheet)
        {
            ...
            if (worksheet.Cells[1, 1].Value.ToString().Trim().Contains("Tên") &&
                 ...
                 worksheet.Cells[1, 15].Value.ToString().Trim().Contains("Số chỗ ngồi") &&
                 worksheet.Cells[1, 16].Value.ToString().Trim().Contains("Biển kiểm soát") &&
                 worksheet.Cells[1, 17].Value.ToString().Trim().Contains("Tải trọng") &&
                 worksheet.Cells[1, 18].Value.ToString().Trim().Contains("Công suất")
                )
            {
                return true;
            }
